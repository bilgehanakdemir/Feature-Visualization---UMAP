...
def init_dataset(path, dataset, batch='train'):  # path = kvs['args'].dataset_root
    if 'CIFAR10' == dataset:
        ...
        if 'train' == batch:
            ...
            raise NotImplementedError
#    elif 'MedNIST' == dataset:
#        if 'train' == batch:
#            batch_list = [
#                ['train', '16019d7e3df5f24257cddd939b257f8c'],
#            ]
#        elif 'test' == batch:
#            batch_list = [
#                ['test', 'f0ef6b0ae62326f3e7ffdfab6717acfd'],
#            ]
#        else:
#            raise NotImplementedError
...
def init_metadata():
    ...
    if 'CIFAR' in kvs['args'].dataset_name:
        ...
        training_img_lst = list(kvs['args'].training_data_loc.glob('*.png'))
        class_names = list(map(lambda x: x.stem.split('_')[0], training_img_lst))
        meta = pd.DataFrame({'Label': class_names, 'Filename': training_img_lst})  # TODO: Add column ID
        idx = 0
