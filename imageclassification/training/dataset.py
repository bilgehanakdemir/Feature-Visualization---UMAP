...
class ImageClassificationDataset(data.Dataset):
    ...
    def __getitem__(self, ind):
        ...
        if self.dataset is None:  # MedMNIST, in init_dataset ds was set None for MedMNIST
            #dimg = cv2.imread((entry.Filename),0)  # here images come from the disk also -1 
            dimg = cv2.imread((entry.Filename),cv2.IMREAD_COLOR)
            #print("dimg shape: ",dimg.shape)
            '''NOTE: the image might be single channel image depending on how it was saved, adjust transformations and CNN model accordingly or (perhaps better #to) convert the loaded image as rgb (check in which order the r, g,and b     channels should be given when working with SOLT and also PyTorch#) '''
        else:  #CIFAR10, CIFAR100
            dimg = self.dataset[indx, :, :, :]  #here images come from a variable initialized in init_dataset
...
def init_dataset(path, dataset, batch='train'):  # path = kvs['args'].dataset_root
    ...
    ds = []
#	  if 'CIFAR10' == dataset or 'CIFAR100' == dataset:
...
#		    ds = ds.reshape(l_dataset, 3, 32, 32).swapaxes(1, 3).swapaxes(1, 2) #CIFAR
    #elif 'MedMNIST' == dataset:
    if 'MedMNIST' == dataset:
		    ds = None  # with MedMNIST we don't want to read the whole dataset into memory prior training and therefore ds can be ignored in this function
		    l_dataset = None
    else:
		    raise NotImplementedError
	
    return ds, l_dataset  # TODO: Remove l_dataset as it seems not to be used anywhere in the project, fix run_training accordingly
...
def init_metadata():
    ...
    if 'CIFAR' in kvs['args'].dataset_name:
        ...
        save_path = "/scratch/project_2006161/cifar-10/metadata/MedMNIST"  # TODO: Compose this using os.path.join(kvs['args'].metadata_root, kvs['args'].dataset_name)
        #if not os.path.exists(save_path):
        #    os.makedirs(save_path)
    ...
    if 'CIFAR10' in kvs['args'].dataset_name:
        cv_split = [x for x in skf.split(kvs['metadata']['Filename'].astype(str),
                                         kvs['metadata']['Label'],
                                         kvs['metadata']['ID'])]
    elif 'MedMNIST' in kvs['args'].dataset_name:  # data is split here in to n cross-validation folds for training
        cv_split = [x for x in skf.split(kvs['metadata']['Filename'].astype(str),
                                         kvs['metadata']['Label'],
                                         kvs['metadata']['ID'])]
